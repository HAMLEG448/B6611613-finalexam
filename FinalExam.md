...
> **ตอบ:** 
>
>ก. ควรใช้ Terraform เป็นการสร้าง resource ของ infrastructure ใน Kubernetes และต้องการให้ state ของระบบชัดเจนว่ามี namespace กับ volume อะไรอยู่
>
>ข. ควรใช้ Ansible เพราะเป็นงานติดตั้งและตั้งค่า software บน server หลายเครื่อง ทำเป็น playbook แล้วรันซ้ำได้
>
>ค. ควรใช้ Ansible เพราะเป็นการแก้ configuration บน server

### ข้อ 3.3 (6 คะแนน)

**Idempotency** คืออะไร และทำไมจึงสำคัญสำหรับ Infrastructure as Code? (อธิบายว่าถ้ารัน script เดิมซ้ำ 2 ครั้งควรเกิดอะไรขึ้น)

> **ตอบ:** Idempotency คือการรันคำสั่งหรือ script ซ้ำหลายครั้งแล้วผลลัพธ์สุดท้ายยังเหมือนเดิม สิ่งนี้สำคัญกับ Infrastructure as Code เพราะเราต้องรัน automation ซ้ำได้อย่างปลอดภัย ถ้ารัน script เดิม 2 ครั้งแล้วสร้าง resource ซ้ำ ระบบจะจัดการยากและอาจเกิด error ได้
...
> **ตอบ:** 
>
> Pod คือหน่วยเล็กสุดที่รันแอปใน Kubernetes
>
> Deployment คือ ตัวควบคุม Pod
>
> Service คือ ตัวกลางให้ผู้ใช้หรือระบบอื่นเรียกหา Pod ได้แบบคงที่
>
> ความสัมพันธ์คือ Deployment สร้างและดูแล Pod ส่วน Service ชี้ไปหา Pod เพื่อให้เรียกใช้งานแอปได้ตลอด
...
> **ตอบ:** ต่างกันที่ “จุดประสงค์ของการตรวจ” ถ้าตั้ง liveness probe ผิด เช่นตรวจถี่เกินไป หรือ timeout สั้นเกิน แอปอาจโดน restart
...
> **ตอบ:** Kubernetes ทำ zero-downtime deployment ได้ด้วย rolling update คือค่อย ๆ สร้าง Pod version ใหม่ขึ้นมาแทน Pod version เก่าทีละชุด ระหว่างนั้น Service ยังส่ง traffic ไปหา Pod ที่พร้อมใช้งานอยู่ ทำให้ผู้ใช้ยังเข้าเว็บได้ ไม่ต้องรอ 15–20 นาทีเหมือนการ SSH ไป restart เอง
>
>ถ้า deploy version ใหม่แล้วมีปัญหา สามารถ rollback กลับไป version ก่อนหน้าได้ ด้วยคำสั่ง
>
...
> **ตอบ:** _[Prometheus เป็นตัวเก็บ metric และ query ข้อมูลจากระบบ ส่วน Grafana เป็นตัวแสดงผล metric ที่ Prometheus เก็บมาเป็น dashboard และกราฟให้ดูง่าย สำหรับเว็บแอปนี้ควรเก็บ metric เช่น response time เพื่อดูว่า API ช้าหรือไม่, HTTP error rate/5xx เพื่อดูว่า backend มีปัญหาหรือไม่, และ CPU/Memory usage เพื่อดูว่า Pod หรือ MySQL ใช้ทรัพยากรสูงจนเสี่ยงล่มหรือไม่]_
...
> **ตอบ:** 
>
>**HTTP 5xx error สูงผิดปกติ**
>
>condition: error 5xx มากกว่า 5% ต่อเนื่อง 5 นาที
>เหตุผล: แปลว่าผู้ใช้อาจเข้าเว็บไม่ได้ หรือ API มีปัญหา
>
>**Pod restart บ่อย**
>
>condition: Pod restart มากกว่า 3 ครั้งใน 10 นาที
>เหตุผล: อาจเกิดจากแอป crash, config ผิด หรือ liveness probe มีปัญหา
>
>สอง alert นี้สำคัญเพราะช่วยให้ทีม ops รู้ปัญหาก่อนที่ผู้ใช้จะร้องเรียน
---
